...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs
                                            ...
                                            if livingGhosts[i+1]]
        ghost_distances = [float("inf") for _ in livingGhostPositionDistributions]
        ghost_positions = [None for _ in livingGhostPositionDistributions]

        # "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()

        if len(livingGhosts)>1:
            i = 0
            for ghost in livingGhosts[1:]:
                if ghost:
                    beliefs = livingGhostPositionDistributions[i]
                    max_belief = -1
                    max_pos = None
                    ...
                    ghost_positions[i] = max_pos
                    ghost_distances[i] = self.distancer.getDistance(pacmanPosition, max_pos)
                    i += 1
            ...
            for i, distance in enumerate(ghost_distances):
                if distance < min_distance:
                    ...
                    closest_ghost_pos = ghost_positions[i]

            min_distance = float("inf")
            best_action = None
            for action in legal:
                newPos = Actions.getSuccessor(pacmanPosition, action)
                distance = self.distancer.getDistance(newPos, closest_ghost_pos)
                if distance < min_distance:
                    min_distance = distance
                    best_action = action
