...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for oldPos, belief in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, probability in newPosDist.items():
                allPossible[newPos] += belief*probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [self.legalPositions[i] for i in range(len(self.legalPositions))
        ...
        for i in range(self.numParticles - len(self.particles)):
            self.particles.append(self.legalPositions[i])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for particle in self.particles:
                ...
                self.weights[particle] += probability

            if self.weights.totalCount() == 0:
                self.initializeUniformly(gameState)

            else:
                self.particles = [util.sample(self.weights) for _ in range(self.numParticles)]
                # for _ in range(self.numParticles):
                #     self.particles.append(util.sample(self.weights))
    ...
    def getBeliefDistribution(self):
        ...
        total = self.belief.totalCount()

        for key in self.belief.keys():
            self.belief[key] = self.belief[key]/total
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        outerLoop = self.legalPositions
        innerLoop = self.legalPositions

        random.shuffle(outerLoop)
        random.shuffle(innerLoop)

        i = 0
        while True:
            for pos1 in outerLoop:
                for pos2 in innerLoop:
        ...
        for particle in self.particles:
            ...
            for i, noisyDistance in enumerate(noisyDistances):
                if noisyDistance is not None:
                    ...
                    probability *= emissionModels[i][distance]
