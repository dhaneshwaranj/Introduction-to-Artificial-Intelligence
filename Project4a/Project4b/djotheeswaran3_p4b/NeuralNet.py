...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        inp = [1]+inActs[:]
        wSum = self.getWeightedSum(inp)
        return self.sigmoid(wSum)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        inp = [1] + inActs[:]  # Adding bias term in the front
        wSum = self.getWeightedSum(inp)
        return self.sigmoidDeriv(wSum)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(self.inSize):
            modification = alpha * inputs[i] * delta
            self.weights[i] = self.weights[i] + modification
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            inter = [percep.sigmoidActivation(output[-1]) for percep in layer]
            output.append(inter)
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) #g'(in)
                error = example[1][outputNum] - lastLayerOutput[outputNum]  # (y - h(x))
                delta = error * gPrime  # as in the slides, for the output layer
            ...
            for layerNum in xrange(self.numHiddenLayers-1, -1, -1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum])  # g'(in)
                    sumProdWD = sum([nextLayer[i].weights[neuronNum+1] * deltas[0][i] for i in range(len(nextLayer))])
                    # Sum the product of weights and the delta in the next layer. Note that the new deltas
                    # are added to the front of the list. So, access the first delta. Also, add 1 for bias
                    delta = gPrime * sumProdWD
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        predictions = nnet.feedForward(example[0])[-1]
        correct = True
        for i in range(len(predictions)):
            if round(predictions[i])!=example[1][i]:
                correct = False
                break
        if correct:
            testCorrect += 1.0
