...
def getPertinentExamples(examples,attrName,attrValue):
    ...
    for i in range(len(examples)):
        entry = examples[i]
        if entry[attrName] == attrValue:
            newExamples.append(entry)
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        ...
        attributeCounts[attrValue] = getClassCounts(newExamples, className)
...
def remainder(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        newExamples = getPertinentExamples(examples, attrName, attrValue)
        p = len(newExamples) / total
        classes = getClassCounts(newExamples, className)
        entropy = setEntropy(list(classes.values()))
        rem += p * entropy
...
def infoGain(examples,attrName,attrValues,className):
    ...
    classes = getClassCounts(examples, className)
    entropy = setEntropy(list(classes.values()))
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        ...
        p = len(newExamples)/n
        classes = getClassCounts(newExamples, className)
        gini += p * giniIndex(list(classes.values()))
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    if len(examples) == 0:
        ...
        same = True
        value = examples[0][className]
        for example in examples:
            if example[className] != value:
                same = False
                break
        ...
        if same:
            ...
            best_attr = remainingAttributes[0]
            ...
            node = Node(best_attr)
            newRemainingAttributes = [x for x in remainingAttributes if x != best_attr]
            newDefaultLabel = getMostCommonClass(examples, className)

            for attrValue in attributeValues[best_attr]:
                newExamples = getPertinentExamples(examples, best_attr, attrValue)

                subTree = makeSubtrees(newRemainingAttributes, newExamples, attributeValues,
                                       className, newDefaultLabel, setScoreFunc, gainFunc)

                node.children[attrValue] = subTree
